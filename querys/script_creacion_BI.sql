...
CREATE VIEW NOT_FOUND.BI_GANANCIAS_MENSUALES_POR_CANAL AS
	SELECT CANAL,T.AÑO, T.MES, SUM(V.TOTAL_VENTA) - SUM(V.COSTO_MEDIO_PAGO) - (SELECT SUM(C.UNIDADES*C.PRECIO) 
...
CREATE VIEW NOT_FOUND.BI_PRODUCTOS_CON_MAYOR_RENTABILIDAD_ANUAL AS
	SELECT NOMBRE_PRODUCTO, T.AÑO, (SUM(PV.UNIDADES * PV.PRECIO) - (SELECT SUM(C.UNIDADES*C.PRECIO) 
																  FROM NOT_FOUND.BI_HECHOS_COMPRAS C JOIN NOT_FOUND.BI_DIMENSION_TIEMPO T2 ON T2.TIEMPO_ID = C.TIEMPO_ID 
																  WHERE C.PRODUCTO_ID = PV.PRODUCTO_ID AND T2.AÑO = T.AÑO GROUP BY C.PRODUCTO_ID)) / SUM(PV.UNIDADES * PV.PRECIO) *100 RENTABILIDAD_ANUAL
	...
	HAVING PV.PRODUCTO_ID IN (SELECT TOP 5 PV2.PRODUCTO_ID 
							  FROM  NOT_FOUND.BI_HECHOS_PRODUCTOS_VENDIDOS PV2
							  GROUP BY PV2.PRODUCTO_ID
							  ORDER BY (SUM(PV2.UNIDADES * PV2.PRECIO) - (SELECT SUM(C.UNIDADES*C.PRECIO) 
																          FROM NOT_FOUND.BI_HECHOS_COMPRAS C
																          WHERE C.PRODUCTO_ID = PV2.PRODUCTO_ID GROUP BY C.PRODUCTO_ID)) / SUM(PV2.UNIDADES * PV2.PRECIO) DESC)
GO
...
CREATE VIEW NOT_FOUND.BI_VALOR_PROMEDIO_ANUAL_DE_ENVIO_POR_PROVINCIA_POR_TIPO_DE_ENVIO AS
	SELECT PROVINCIA, TIPO_ENVIO, AÑO, AVG(PRECIO_MEDIO_ENVIO) AS VALOR_PROMEDIO_DE_ENVIO
	FROM NOT_FOUND.BI_HECHOS_VENTAS V JOIN NOT_FOUND.BI_DIMENSION_TIEMPO T ON T.TIEMPO_ID = V.TIEMPO_ID
...
CREATE VIEW NOT_FOUND.BI_AUMENTO_PROMEDIO_DE_PRECIOS_ANUAL_DE_PROVEEDORES AS
	SELECT RAZON_SOCIAL, PRODUCTO_ID, AÑO, (MAX(PRECIO) - MIN(PRECIO))/MIN(PRECIO) AS AUMENTO_PROMEDIO_DE_PRECIO
...
CREATE VIEW NOT_FOUND.BI_3_PRODUCTOS_CON_MAYOR_REPOSICION_POR_MES AS
	SELECT NOMBRE_PRODUCTO, T.MES, T.AÑO, SUM(UNIDADES) CANT_REPOSICION
	FROM NOT_FOUND.BI_HECHOS_COMPRAS C JOIN NOT_FOUND.BI_DIMENSION_TIEMPO T ON T.TIEMPO_ID = C.TIEMPO_ID
									   JOIN NOT_FOUND.BI_DIMENSION_PRODUCTO P ON P.PRODUCTO_ID = C.PRODUCTO_ID
	GROUP BY C.PRODUCTO_ID, NOMBRE_PRODUCTO, T.MES, T.AÑO
	HAVING C.PRODUCTO_ID IN (SELECT TOP 3 PRODUCTO_ID FROM NOT_FOUND.BI_HECHOS_COMPRAS GROUP BY PRODUCTO_ID ORDER BY SUM(UNIDADES) DESC)
GO
