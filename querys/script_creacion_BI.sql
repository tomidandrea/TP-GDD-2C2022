...
CREATE VIEW NOT_FOUND.BI_PRODUCTOS_CON_MAYOR_RENTABILIDAD_ANUAL AS
	SELECT NOMBRE_PRODUCTO, T.AÑO, (SUM(PV.UNIDADES * PV.PRECIO) - (SELECT SUM(C.UNIDADES*C.PRECIO) 
																  ...
																  GROUP BY C.PRODUCTO_ID)) / SUM(PV.UNIDADES * PV.PRECIO) *100 RENTABILIDAD_ANUAL
	...
	HAVING PV.PRODUCTO_ID IN (SELECT TOP 5 PV2.PRODUCTO_ID 
						   ...
						   -- WHERE T3.AÑO = T.AÑO
							  ...
							  ORDER BY (SUM(PV2.UNIDADES * PV2.PRECIO) - (SELECT SUM(C.UNIDADES*C.PRECIO) 
																		  ...
																		  GROUP BY C.PRODUCTO_ID)) / SUM(PV2.UNIDADES * PV2.PRECIO) DESC)
GO
...
CREATE VIEW NOT_FOUND.BI_PORCENTAJE_DE_ENVIOS_POR_PROVINCIA AS
	SELECT PROVINCIA, MES, AÑO, CONVERT(DECIMAL(18,0),COUNT(*)) / (SELECT COUNT(*) 
